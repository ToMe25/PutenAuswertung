# Ergebnis Dateien
Dieses Programm schreibt die Ergebnisse der Verarbeitung der [Input Daten](input.md) in zwei Ergebnisdateien.  
...
Wie auch die [Input Dateien](input.md) sind die Ergebnisdateien im [CSV Format](formats.md#csv).

**Achtung:** Das Programm überschreibt momentan die Ergebnisdateien ohne Nachfrage.
...
Ein Aufenthalt kann mehrere Tage andauern, und ist erst als beendet angesehen wenn entweder
 1. die Pute in einem anderen Bereich registriert wurde, oder
 2. ein Zeitraum ohne Aufzeichnungen beginnt.
