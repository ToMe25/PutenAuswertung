...
public enum Argument {
	...
	HELP('h', (short) 6, "help") {
		...
		public void onReceived(Arguments inst, String val) {
			...
			boolean jar = false;
			String classRes = this.getClass().getName().replace('.', '/') + ".class";
			URL resource = this.getClass().getClassLoader().getResource(classRes);
			if (resource != null) {
			...
			if (stack.length > 0) {
				...
				if (mainElement.getMethodName().equals("main")) {
					...
					LogHandler.print_debug_info("Thread name: %s, Thread id: %d, Init class: %s, Init method: %s",
							...
							mainElement.getClassName(), mainElement.getMethodName());
				}
			} else {
